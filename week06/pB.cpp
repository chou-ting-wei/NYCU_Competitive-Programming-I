...
int chk(vector<double> tmp, double inp, double big){
    tmp.eb(inp);
    sort(all(tmp));
    if(tmp[1] + tmp[2] + tmp[3] - big < eps) return 1;
    return 0;
}
...
int32_t main(){
    ...
    if(v[1] + v[2] + v[3] - n < eps){
        cout << "infinite" << endl;
    }
    else{
        int flg = 0;
        double ans;
        for(double i = v[0]; i <= v[3]; i += 0.01){
            if(chk(v, i, n)){
                flg = 1;
                ans = i;
            }
        }
        if(flg) cout << fixed << setprecision(2) << ans << endl;
